...
Definition FSig {B : Empty_set -> Type} : T (sigT B) Empty_set.
Proof. refine (
{| to := @projT1 _ B
...
Proof. refine (
{| to := fun x => match x with existT tt p => p end
 ; from := existT _ tt
|}).
Proof. 
...
Lemma FFunc {B : Type} : T (Empty_set -> B) unit.
Proof.
refine (
{| to   := fun _ => tt
 ; from := fun _ => Empty_set_rect (fun _ => B) |}); intros.
apply functional_extensionality. intros. inversion x.
destruct b. reflexivity.
Defined.

Lemma TFunc {B : Type} : T (unit -> B) B.
Proof.
refine (
{| to   := fun f => f tt
